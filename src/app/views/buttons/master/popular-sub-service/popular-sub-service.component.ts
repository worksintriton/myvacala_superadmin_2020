...
@Component({
  selector: 'app-popular-sub-service',
  templateUrl: './popular-sub-service.component.html',
  styleUrls: ['./popular-sub-service.component.scss']
})
export class PopularSubServiceComponent implements OnInit {
  ServiceName: any;
  List: any;
  serviceError = false;
  serviceErrorMsg: any;
  ...
  vechiclt_type_select() {
    ...
    if (this.VehicleType != undefined) {
      this.mainservice_list = this.mainservice_list.filter(x => (x.Vehicle_Type_id._id == this.VehicleType && x.Popular == true));
